...
export function FieldsClient() {
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        const url = selectedField 
            ? `/api/settings/employee-fields/${selectedField.id}`
            : '/api/settings/employee-fields';
        const method = selectedField ? 'PUT' : 'POST';

        // For new fields, calculate order
        const order = selectedField?.order ?? (fields.length > 0 ? Math.max(...fields.map(f => f.order)) + 1 : 1);
    ...
    const onDeleteConfirm = async () => {
        ...
        setActionLoading(true);
        try {
            const response = await fetch(`/api/settings/employee-fields/${selectedField.id}`, { method: 'DELETE' });
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            const updatePromises = newOrder.map((field, index) =>
                fetch(`/api/settings/employee-fields/${field.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
            ...
            try {
                ...
                await refreshData();
                 ...
                 await refreshData(); // Revert to original order from DB
            }
        }
    };
