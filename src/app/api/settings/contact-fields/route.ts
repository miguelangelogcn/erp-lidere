...
// GET /api/settings/contact-fields
export async function GET() {
    try {
        ...
        console.error("Erro ao buscar campos customizáveis de contato:", error);
        return NextResponse.json({ error: "Erro interno do servidor." }, { status: 500 });
    }
}

// POST /api/settings/contact-fields
export async function POST(request: Request) {
    try {
        ...
        const newFieldRef = await adminDb.collection("contactCustomFields").add(validation.data);

        return NextResponse.json({ id: newFieldRef.id, ...validation.data }, { status: 201 });

    } catch (error) {
        console.error("Erro ao criar campo customizável de contato:", error);
        return NextResponse.json({ error: "Erro interno do servidor." }, { status: 500 });
    }
}
