...
// PUT /api/settings/contact-fields/[fieldId]
export async function PUT(request: Request, { params }: RouteParams) {
    ...
    try {
        ...
        const fieldRef = adminDb.collection("contactCustomFields").doc(fieldId);
        await fieldRef.update(validation.data);

        return NextResponse.json({ message: "Campo atualizado com sucesso." });

    } catch (error) {
        console.error(`Erro ao atualizar campo de contato ${fieldId}:`, error);
        return NextResponse.json({ error: "Erro interno do servidor." }, { status: 500 });
    }
}

// DELETE /api/settings/contact-fields/[fieldId]
export async function DELETE(request: Request, { params }: RouteParams) {
    ...
    try {
        ...
        console.error(`Erro ao excluir campo de contato ${fieldId}:`, error);
        return NextResponse.json({ error: "Erro interno do servidor." }, { status: 500 });
    }
}
